...
namespace rad::hdf5tree {
  ...
  { 
    ...
    len = H5Iget_name(gid, groupName, stringBufferSize);
    ...
    for(int idx=0; idx<nobj; idx++) {
      len = H5Gget_objname_by_idx(gid, (hsize_t)idx, memberName, (size_t)stringBufferSize);
  ...
  void Tree::load(std::string filename) {
    ...
    scanGroup(gid, root);
    status = H5Fclose(fid);
    if(status) throw runtime_error("Unable to close file");
    cout << "done" << endl;
  }
